...
class GSN_Sampler(nn.Module):
    ...
    def __init__(
        ...
        super(GSN_Sampler, self).__init__()
    ...
    def sampling(self, n_samples=5, data_dim=2, solver='euler',  params=None, device='cuda'):
        ...
        for t in timestp: 
            ...
            with torch.no_grad():
                x_noisy = self.add_noise(sample)
                sample = self.model(x_noisy)
...
def gsn_sampling(expr_id, n_timestep_smpl=-1, n_sel_time=10, n_samples=36, train_name=None, test_name=None):
    ...
    gsn = GSN_Sampler(model=model, dataloader=None, n_timestep_smpl=n_timestep_smpl, expr_id=expr_id)
    print(f'\n {expr_id}\n')
    gsn.sampling(n_samples=n_samples, data_dim=x_batch.shape[1:], params=params, device=device)
