...
from diffusion_flow_matching_sampling import FlowMatching_Sampler

class FlowMatching_Model(nn.Module):
    ...
    def __init__(
        ...
        super(FlowMatching_Model, self).__init__()
    ...
    def train(self, n_epochs=5, lr=0.01, gamma=0.001, device='cuda'):
        ...
        for epoch in epochs:
            ...
            for itr, x in enumerate(self.dataloader):
                ...
                grad, predicted_grad = self.train_flow_step(x)
            ...
            if True: 
                ...
                if (epoch + 1)% params.save_freq == 0 and params.validation:
                    self.save_result(epoch, loss_hist, x.shape[1], gamma)
    ...
    def train_flow_step(self, x):
        ...
        # t = torch.randint(0, self.n_timesteps, size=[1]).reshape(x.shape[0], 1).to(x.device)
            ...
            x_noisy, grad = self.add_noise_flow(x, t)
            
        predicted_grad = self.model(x_noisy, t)

        return grad, predicted_grad
    
    def save_result(self, epoch, loss_hist, data_dim, gamma):
        
        sampler = FlowMatching_Sampler(model=self.model, betas=self.betas, n_timestep_smpl=n_timestep_smpl, training=True)
        samples_zero, intermediate_smpl = sampler.sampling(n_samples=params.n_samples, gamma=gamma, data_dim=data_dim, 
...
if __name__=='__main__':
    ...
    flow_matching = FlowMatching_Model(model=model, dataloader=dataloader, betas=betas, n_timesteps=n_timesteps, expr_id=expr_id)
    print(f'\n {expr_id}\n')
    ...
    flow_matching.train(n_epochs=n_epochs, lr=lr, gamma=gamma, device=device)
