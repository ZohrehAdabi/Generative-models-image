...
class DDPM_Sampler(nn.Module):
    ...
    def sampling(self, n_samples=5, data_dim=2, hidden_size=2, grid_size=10, normalize=False, scaler=None, params=None, device='cuda'):

        self.model.to(device)
        self.model.eval()

        self.idx_data = hidden_size // 2

        sample = torch.randn(n_samples, data_dim+hidden_size).to(device)
        timesteps = torch.arange(self.n_timestep_smpl-1, -1, -1)
        intermediate_smpl = np.empty([self.n_timestep_smpl+1, n_samples, data_dim])
        intermediate_smpl[-1] = sample[:, self.idx_data:-self.idx_data].cpu()
        ...
        grid_samples = self.add_hidden_padding(grid_samples, hidden_size)
        ...
        for t in timestp: 
            ...
            with torch.no_grad():
                ...
                nois_norm_list.append([torch.linalg.norm(predicted_noise[:, self.idx_data:-self.idx_data], axis=1).mean().cpu().numpy()])
                
                pred_noise = self.model(grid_samples, torch.repeat_interleave(t, grid_size**2).reshape(-1, 1).long().to(device))
                noises_grid[t, :, :] = pred_noise[:, self.idx_data:-self.idx_data].cpu().numpy()
            
            if normalize:
                 intermediate_smpl[t, :, :] = scaler.inverse_transform(sample[:, self.idx_data:-self.idx_data].cpu())
            else:
                intermediate_smpl[t, :, :] = sample[:, self.idx_data:-self.idx_data].cpu()
    ...
    def add_hidden_padding(self, x, hidden_size):
        ...
        if x.ndim == 2:    
            padding_random = torch.randn(x.shape[0], x.shape[1] + hidden_size)
            padding_random[:, self.idx_data:-self.idx_data] = x
...
if __name__=='__main__':
    ...
    model = select_model_diffusion(model_info=model_name, data_dim=data_dim+hid_inp_size, time_dim=time_dim, n_timesteps=n_timesteps, device=device)
    ...
    print(f'\n {expr_id}\n')
    ddpm.sampling(n_samples=n_samples, data_dim=data_dim, hidden_size=hid_inp_size, normalize=params.normalize, scaler=scaler, params=params, device=device)
