...
class DDPM_Sampler(nn.Module):
    ...
    def sampling(self, n_samples=5, data_dim=2, hidden_size=2, params=None, device='cuda'):

        self.model.to(device)
        self.model.eval()
        dim = [data_dim[0], data_dim[1] + 2*hidden_size, data_dim[2] + 2*hidden_size]
        sample = torch.randn(n_samples, *dim).to(device)
        timesteps = torch.arange(self.n_timestep_smpl-1, -1, -1)
        
        intermediate_smpl = np.empty([self.n_timestep_smpl+1, n_samples, *dim[::-1]])
        intermediate_smpl[-1] = denormalize(sample.cpu().numpy())
        
        noises = np.empty([self.n_timestep_smpl, n_samples, *dim[::-1]])
        nois_norm_list = []      
        ...
        if True:
            sample_zero = sample_zero[:, hidden_size:-hidden_size, hidden_size:-hidden_size, :] 
            intermediate_smpl = intermediate_smpl[:, :, hidden_size:-hidden_size, hidden_size:-hidden_size, :]
    ...
    def evaluation(self, n_samples=5, real_dataloader=None, data_dim=2, batch_size=10, params=None, device='cuda'):
        ...
        if not params.use_torchmetric:
            ...
            fid = FrechetInceptionDistance(feature=64, reset_real_features=False, input_img_size=(3, 299, 299), normalize=False)
...
def ddpm_sampling(expr_id, n_timestep_smpl=-1, n_sel_time=10, n_samples=36, evaluate=False, train_name=None, test_name=None):
    ...
    print(f'\n {expr_id}\n')
    ddpm.sampling(n_samples=n_samples, data_dim=x_batch.shape[1:], hidden_size=params.hid_inp_size, params=params, device=device)
    if evaluate:
        ddpm.evaluation(n_samples=n_samples, real_dataloader=dataloader, data_dim=x_batch.shape[1:], params=params, device=device)
...
if __name__=='__main__':
    ...
    ddpm = DDPM_Sampler(model=model, dataloader=None, betas=betas, n_timestep_smpl=n_timestep_smpl, expr_id=expr_id)
    print(f'\n {expr_id}\n')
    ddpm.sampling(n_samples=n_samples, data_dim=x_batch.shape[1:], hidden_size=hid_inp_size, params=params, device=device)
