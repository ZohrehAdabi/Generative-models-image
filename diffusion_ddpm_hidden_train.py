...
class DDPM_Model(nn.Module):
    ...
    def save_result(self, epoch, loss_hist, data_dim):
        ...
        sample_zero, intermediate_smpl, noises_grid, noise_norm = sampler.sampling(n_samples=params.n_samples, data_dim=data_dim, \
                                                                                    hidden_size=params.hid_inp_size, grid_size=params.grid_size, 
        ...
        if params.save_hdf:
            ...
            # dfims = pd.DataFrame(samples_zero, columns=['x', 'y'])
            # with pd.HDFStore(file_sample_zero_all, 'a') as hdf_store_samples_zero:
            #     hdf_store_samples_zero.append(key=f'df/sample_zero_epoch_{epoch + 1:06}', value=dfims, format='t')
...
if __name__=='__main__':
    ...
    if params.save_hdf:
        file_loss, file_sample, file_sample_zero_all, df_loss_per_itr = \
                create_hdf_file_training(params.save_dir, expr_id, dataset, n_timestep_smpl,  params.n_sel_time, params.grid_size, params.n_samples)
